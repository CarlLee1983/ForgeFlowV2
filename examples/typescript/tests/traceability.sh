...
expect_success 'the real TypeScript order-total mapping' \
  "$checker" "$acceptance_file" "$test_file"
expect_failure 'a missing acceptance input' \
  "$checker" "$fixture_dir/missing.md" "$test_file"
expect_failure 'a missing test input' \
  "$checker" "$acceptance_file" "$fixture_dir/missing.ts"
...
: >"$fixture_dir/empty-test.ts"
expect_failure 'a test source with no identifiers' \
  "$checker" "$acceptance_file" "$fixture_dir/empty-test.ts"
...
cp "$test_file" "$fixture_dir/duplicate-test.ts"
printf '\ntest("AC-01: duplicate", () => {});\n' \
  >>"$fixture_dir/duplicate-test.ts"
expect_failure 'a duplicate executable test identifier' \
  "$checker" "$acceptance_file" "$fixture_dir/duplicate-test.ts"

sed 's/AC-06:/AC-09:/' "$test_file" >"$fixture_dir/mismatched-test.ts"
expect_failure 'mismatched acceptance and test identifier sets' \
  "$checker" "$acceptance_file" "$fixture_dir/mismatched-test.ts"

printf 'TypeScript traceability regression tests passed\n'
