...
acceptance_file=${1:-"$example_dir/specs/stories/TYP-001-order-total/acceptance.md"}
test_file=${2:-"$example_dir/test/order-total.test.ts"}
...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/praxisbound-typescript-traceability.XXXXXX")
...
sed -n 's/^[[:space:]]*[-*] \[ \] \(AC-[0-9][0-9]*\):.*/\1/p' \
  "$acceptance_file" | sort >"$acceptance_ids"
# Test identifiers are intentionally the first text in single-line test names.
sed -n 's/^[[:space:]]*test("\(AC-[0-9][0-9]*\):.*/\1/p' \
...
identifier_count=$(wc -l <"$acceptance_ids" | tr -d '[:space:]')
printf 'TypeScript traceability passed: %s identifiers\n' "$identifier_count"
