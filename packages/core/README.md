...
installed membership, snapshot, checksum, and no-follow path observations. It
returns `ACTIVATION_PREVIEW`, `ACTIVATION_UNCHANGED`, a conflict/refusal, or an
ordered content-addressed plan whose activation snapshot is last.
`evaluateActivationMutation(plan, observation)` uses the same immutable
mutation-execution observation contract as Init while assigning only
activation-specific outcomes. Core performs no filesystem, process, Git,
